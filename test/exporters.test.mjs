...
test('createMusicXml: 小節数、コード名エスケープ、2段譜を生成',()=>{
  ...
  assert.match(xml,/<staff-tuning line="1"><tuning-step>E<\/tuning-step><tuning-octave>2<\/tuning-octave>/);
...
test('createMusicXml: 標準ギターの楽器情報とTAB弦順を明示する',()=>{
  ...
  assert.match(xml,/<midi-program>25<\/midi-program>/);
  assert.match(xml,/<staff-tuning line="1"><tuning-step>E<\/tuning-step><tuning-octave>2<\/tuning-octave>/);
  assert.match(xml,/<staff-tuning line="6"><tuning-step>E<\/tuning-step><tuning-octave>4<\/tuning-octave>/);
});
